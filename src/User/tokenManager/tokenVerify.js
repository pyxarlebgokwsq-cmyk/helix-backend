...
async function verifyToken(req, res, next) {
  ...
  if (
    ...
    return authErr();
  let token = req.headers["authorization"].replace("bearer eg1~", "");
  try {
    let decodedToken = jwt.decode(token);
    if (decodedToken.am == "client_credentials") {
      if (!global.clientTokens.find((i) => i.token == `eg1~${token}`))
      ...
      let AT = global.accessTokens
        .find(
          (i) =>
            jwt.decode(i.token.replace("eg1~", "")).clid == decodedToken.clid
        )
        .token.replace("eg1~", "");
      decodedToken = jwt.decode(AT);
      token = AT;
    }
    ...
    if (req.user.banned)
      return Utils.createError(
        ...
        "You have been permanently banned from Helix.",
        [],
        -1,
        undefined,
        400,
...
async function verifyClient(req, res, next) {
  ...
  try {
    ...
    if (accessIndex != -1 || clientIndex != -1) Utils.UpdateTokens();
    return authErr();
  }
}
...
export { verifyToken, verifyClient, DateAddHours };