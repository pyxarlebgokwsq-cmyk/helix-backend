...
if (process.env.CLOUD_MODE === "1") {
  logger.xmpp("XMPP en mode cloud (port partage avec l'API)");
  global.__handleCloudXmpp = (req, socket, head) => {
    wss.handleUpgrade(req, socket, head, (ws) => {
      wss.emit("connection", ws, req);
    });
  };
  if (typeof global.__flushPendingXmppUpgrades === "function")
    global.__flushPendingXmppUpgrades();
} else {
  const xmppServer = app.listen(port);
  xmppServer.on("upgrade", (req, socket, head) => {
    wss.handleUpgrade(req, socket, head, (ws) => {
      wss.emit("connection", ws, req);
    });
  });
